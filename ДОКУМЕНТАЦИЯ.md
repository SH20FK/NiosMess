...
Команды BotCreator:
- `/newbot <имя> <username>` — создать бота (username должен оканчиваться на `bot`)
- `/mybots` — список ботов
...
При нажатии фронтенд отправляет `callback_query` через WebSocket, а сервер записывает update для бота.
