...
async def serialise_message(msg: Message, db: AsyncSession) -> dict:
    content = None
    if msg.is_e2ee:
        # For E2EE messages, return encrypted content as-is (client will decrypt)
        content = None  # Don't decrypt on server
    elif msg.encrypted_content and not msg.is_deleted:
        try:
    ...
    if msg.reply_markup:
        try:
            import json
            reply_markup = json.loads(msg.reply_markup)
        except Exception:
            reply_markup = None

    # Для медиафайлов используем защищенный endpoint
    media_url = None
    if msg.media_path:
        media_url = f"https://ni-os.ru/api/media/{msg.media_path}"
    ...
    return {
        ...
        "content": content,
        "is_e2ee": msg.is_e2ee,
        "e2ee_content": msg.e2ee_content if msg.is_e2ee else None,
