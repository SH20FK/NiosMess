...
import { createGroup, createChannel } from "../../lib/api";
...
export function CreateGroupScreen({ onNavigate, session }: CreateGroupScreenProps) {
  ...
  const handleCreate = async () => {
    ...
    try {
      if (groupType === "group") {
        await createGroup({ token: session.token, owner: session.username, name: name.trim() });
      } else {
        await createChannel({ token: session.token, owner: session.username, name: name.trim() });
      }
      onNavigate("main");
    } catch (err: any) {
      setError(err?.detail || "  ");
        ...
        <div className="p-6 border-t" style={{ borderColor: "var(--nm-border)" }}>
          <div className="flex gap-3">
            ...
            <button
              ...
              disabled={loading}
              className="flex-1 px-6 py-3 rounded-xl transition-all duration-200 hover:scale-[1.02] disabled:opacity-60"
              style={{
