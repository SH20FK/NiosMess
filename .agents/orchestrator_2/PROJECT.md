...
| 11 | Comprehensive E2E Testing Suite | Tier 1 (Feature Coverage), Tier 2 (Boundary & Corner), Tier 3 (Cross-Feature Combinations), Tier 4 (Real-World Application Scenarios) | Test Track / M4 | ORIGINAL_REQUEST Acceptance |
| 12 | Adversarial Hardening (Tier 5) | White-box stress testing, gap audit, edge case inputs, rapid resend/expiry handling | M4 | Project Pattern |
| 13 | SemVer Versioning & Audit | Automatic version bump in `pubspec.yaml` (SemVer) and forensic integrity validation | M4 / Audit | AGENTS.md |
...
| M2 | Login, Registration & Route Alignment | `login_screen.dart`, `register_screen.dart`, `m3_auth_text_field.dart`, biometric auth, `/legal/terms` route alias, widget tests | none | IN_PROGRESS |
| M3 | Verification & Security Flows | `two_fa_screen.dart`, `verify_email_screen.dart`, `reset_password_*.dart`, `m3_otp_input_field.dart`, `m3_resend_countdown_timer.dart`, widget tests | none | IN_PROGRESS |
| E2E | E2E Testing Suite Track | Fix `test/auth_e2e_flow_test.dart` and build systematic 4-tier E2E test suite covering all auth/onboarding flows -> `TEST_READY.md` | M1, M2, M3 | IN_PROGRESS |
| M4 | Final E2E Pass & Adversarial Hardening | Pass 100% E2E test suite, Tier 5 Adversarial Coverage Hardening, Static Analysis (0 errors), SemVer bump, Forensic Audit | E2E, M1, M2, M3 | PLANNED |
...
### State Layer ↔ UI
- `sessionProvider`: `completeOnboarding()`
- `authProvider`: `login(identifier, password)`, `register(name, username, email, password)`, `verifyEmail(email, code)`, `verifyTwoFa(identifier, code)`, `requestPasswordReset(email)`, `confirmPasswordReset(email, code, newPassword)`
- `uiSettingsProvider`: `setLocaleCode(code)`
