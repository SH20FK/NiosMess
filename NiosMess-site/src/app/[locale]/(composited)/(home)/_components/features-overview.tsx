...
export default function FeaturesOverview({features, className, smoothnessCoefficient = 0.92, leftOffset = 0, nextDelay = 5000}: FeaturesOverviewProps) {
    ...
    return (
        <div className={"relative flex flex-col gap-4 pt-6 lg:pt-10 " + className || ""}>
                ...
                {features.map(({name}, i) => (
                    <a
                        ...
                        className={"min-w-max max-h-20 whitespace-pre-line " +
                            (slide !== i ? "text-neutral-400" : "text-white")}
                        key={i}
                    >
                        {name}
                    </a>
            ...
            <div className="absolute inset-x-0 bottom-16 lg:bottom-0 top-autp w-full h-48 bg-gradient-to-t from-[#221829] to-transparent pointer-events-none rounded-b-3xl" />
            <div className="absolute flex flex-row gap-2 p-3 rounded-full mx-auto bg-[#120b16] bottom-8 lg:-bottom-16 w-fit left-1/2 -translate-x-1/2">
                {features.map((_, i) => (
                    <button
                        key={i}
                        className={"w-2 h-2 rounded-full cursor-pointer " + (slide === i ? "bg-primary-200" : "bg-white/20")}
                        onClick={() => setSlide(i)}
                    />
                ))}
            </div>
